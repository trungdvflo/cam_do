...
module.exports = class user_branch_ctrl extends base_ctrl {
    ...
    find () {
        var t = this;
        var data = {};
        var ubranchM = new userBranchModel(t.db);
        var cond = [];
        if(Utils.isSet(t.req.body.user_id)){
            cond['user_id'] = t.req.body.user_id;
        }
        if(Utils.isSet(t.req.body.user_id)){
            cond['user_id'] = t.req.body.user_id;
        }
        var orderby = [];
        ubranchM.findAsync(cond, orderby).then(result => {
            data = result;
    ...
    add () {
      ...
      if(!data || !data.user_id || !data.branch_id){
          logger.error('error data request', this.constructor.name+".save");
          t.responeData(data, false, 300, "Lỗi dữ liệu gởi");
          return;
      }

      var branchM = new userBranchModel(t.db);
      branchM.insertAsync(data).then(result =>{
          t.responeData(result, true, 200, "success");
      })
      .catch(err =>{
    ...
    remove () {
      ...
      var branchM = new userBranchModel(t.db);
      branchM.deleteAsync(data.id).then(result =>{
          t.responeData(result, true, 200, "success");
      })
      .catch(err =>{
          logger.error(err, this.constructor.name+".save");
