/**
 * user_group controller.
 * create 2017/12/25
 * @author trungdv
 * @version $Id$
 * @copyright 2017 MIT
 */ 

const Constants = require('../lib/constants.js');
const Utils = require('../lib/utils.js');
const AppDebug = require('../lib/appDebug.js');
const cacheManager = require('../lib/cacheManager.js');
const base_ctrl = require('../controllers/base_ctrl.js');
var baseModel = require('../models/baseModel.js');
const logger = require('../lib/logger.js');

module.exports = class user_group_ctrl extends base_ctrl {
    beforeFilter(db, req, res, next, session){
        super.beforeFilter(db, req, res, next, session);
    }
    index () { 
        var data = {}
        this.responeData(data, true, 200, "success");
    }

    /**
     * find user action
     * @author: trungdv
     */
    get_group (id) {
        var t = this;
        var data = {};
        if(!id){
            logger.error('miss id', this.constructor.name+".get_group");
            t.responeData(data, false, 300, "Lỗi dữ liệu");
        }
        
        var user_groupM = new baseModel(t.db, 'acl_user_group', 'id');
        var cond = [];
        cond['id'] = id;
        cond['deleted'] = 0;
        var orderby = [];
        user_groupM.firstAsync(cond, orderby).then(result => {
            data = result;
            t.responeData(data, true, 200, "success");
        })
        .catch(err => {
            logger.error(err, this.constructor.name+".get_group");
            t.responeData(data, false, 301, "Không thể tìm");
        })
    }
    /**
     * find user action
     * @author: trungdv
     */
    find () {
        var t = this;
        var data = {};
        
        var user_groupM = new baseModel(t.db, 'acl_user_group', 'id');
        var cond = [];
        if(Utils.isSet(t.req.body.name)){
            cond['name LIKE'] = t.req.body.name+'%';
        }
        if(Utils.isSet(t.req.body.deleted)){
            cond['deleted'] = t.req.body.deleted;
        }
        //cond['deleted'] = 0;
        var orderby = [];
        orderby['name']  = 'ASC';
        user_groupM.findAsync(cond, orderby).then(result => {
            data = result;
            t.responeData(data, true, 200, "success");
        })
        .catch(err => {
            logger.error(err, this.constructor.name+".find");
            t.responeData(data, false, 301, "fail");
        })
    }
    /**
     * save action
     * @author: trungdv
     */
    save () {
        var data = this.req.body;
        var t = this;
        if(!t.hasPermission('user_management')){
            logger.error('permission user_management', this.constructor.name+".save");
            return t.responeData(data, false, 1005, "Access denined!");
        }
        if(!data || !data.name || !data.value){
            logger.error('error data request', this.constructor.name+".save");
            t.responeData(data, false, 300, "Lỗi dữ liệu gởi");
            return;
        }
        AppDebug.log(data);
        var user_groupM = new baseModel(t.db, 'acl_user_group', 'id');
        if(!data.id){
            // check duplicate
            var conds = {
                'OR': {
                    name: data.name,
                    value: data.value
                }
            }
            user_groupM.firstAsync(conds).then(res => {
                if(res && res.id){
                    return t.responeData(data, false, 300, "Tên hoặc giá trị nhóm đã tồn tại");
                }
                user_groupM.insertAsync(data).then(result =>{
                    t.responeData(result, true, 200, "success");
                })
                .catch(err =>{
                    logger.error(err, this.constructor.name+".save");
                    t.responeData(data, false, 300, "không thể insert");
                })
            }).catch(err =>{
                logger.error(err, this.constructor.name+".save");
                t.responeData(data, false, 300, "không thể insert");
            })
        }else{
            user_groupM.updateAsync(data).then(result =>{
                t.responeData(result, true, 200, "success");
            })
            .catch(err =>{
                logger.error(err, this.constructor.name+".save");
                t.responeData(data, false, 300, "không thể update");
            })
        }
    }

}